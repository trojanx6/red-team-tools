  ...
  `8.`888b             ,8',8 8888       `8b  8 8888     `88  8 8888        `88. 8 8888     `88 8 8888     `88  8 8888         8.`8888.   Y8  8.`8888.   Y8                                                                                                                               
   `8.`888b     .b    ,8' 88 8888        `8b 8 8888     ,88  8 8888         `88 8 8888     ,88 8 8888     ,88  8 8888         `8.`8888.      `8.`8888.                                                                                                                                   
    ...
    : :  :   :: : :    : :: ::    :   : :          
            ...
            soup_2 = bs(istek_2.text,features="xml")
...
Hem kullanici adinu hemde sifreyi bilmiyorsaniz 3 e basiniz 
    ...
    def SubDomainScanner(self):
        ...
        targetUrl = str(input("[ + ] Url Adresini giriniz : "))
        target = ""

        try:
            target = targetUrl.split("//www.")[1].split("/")[0]
        except IndexError:
            target= targetUrl.split("//")[1].split("/")[0]
        ...
        with open(wordlistPath, mode="r", encoding="utf-8") as wlist:
            ...
            for sub in lines:
                try:
                    url = f"{targetUrl.split('//')[0]}//{sub}.{target}"

                    __requests = requests.get(url=url)
                    if __requests.status_code == 200:
                        print(Fore.GREEN+"]]- SubDomain -[[ -> "+Fore.RESET+__requests.url)

                except requests.ConnectionError:
                        print(Fore.RED+" <---- Url Bulunamadı")
...
""")
            ...
            if islem == 1:
                SiberAtayTools.sql_dork()
                
            elif islem == 2:
                SiberAtayTools.wordpress()
                ...
                SiberAtayTools.nikto()
                
            elif islem == 21:
                SiberAtayTools.beef()
              
            elif islem == 22:
                SiberAtayTools.wireshark()
               
            elif islem == 23:
                SiberAtayTools.dnsmap()
                
            elif islem == 24:
                SiberAtayTools.Hydra()
                
            elif islem == 25:
                SiberAtayTools.Hashcat()
